...
class IndonesiaDate {
    ...
    public function setDate3($tanggal) {
        if (!empty($tanggal)) {
            $exp = explode('-', $tanggal);
            $tglExp = explode(' ', $exp[2]);
            $bln = $this->arrBulanShort[(int) $exp[1]];
            $date = $tglExp[0] . ' ' . $bln . ' ' . $exp[0];
            return $date;
        }
    }
    ...
    public function setShortDateTime($tanggal) {
        if (!empty($tanggal)) {
            $exp = explode(' ', $tanggal);
            $exp1 = explode('-', $exp[0]);
            $tglExp = explode(' ', $exp1[2]);
            $bln = $this->arrBulanShort[(int) $exp1[1]];
            $date = $tglExp[0] . ' ' . $bln . ' ' . $exp1[0] . ' ' . $exp[1];
            return $date;
        }
    }
    ...
    public function setDayDate($tanggal) {
        if (!empty($tanggal)) {
            $exp = explode('-', $tanggal);
            $bln = $this->arrBulan[(int) $exp[1]];
            $x = mktime(0, 0, 0, date($exp[1]), date($exp[2]), date($exp[0]));
            $xHari = date('l', $x);
            switch ($xHari) {
                case 'Sunday': {
                        $namaHari = "Minggu";
    ...
    public function setDateRange($dateMulai, $dateSelesai) {
        if (!empty($dateMulai) && !empty($dateSelesai)) {
